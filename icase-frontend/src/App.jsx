...
export default function App() {
  ...
  const [loadingOverlay, setLoadingOverlay] = useState({ visible: false, message: "" });

  // Función reutilizable para mapear proyectos de MongoDB al modelo de estado de la aplicación
  const mapBackendProject = useCallback((p) => {
...
@enduml`;
    ...
    const hasRealDiagrams = (diags || []).length > 0;
    const isProcessed = Boolean(
      (p.estado_fase && p.estado_fase !== "insumos_pendientes") ||
      hasRealRequirements ||
      hasRealDiagrams
    );

    const cleanRawName = (p.nombre || "")
      .replace(/^(Sistema de|Sistema para|Sistema|Software de|Software para|Aplicación de|Plataforma de)\s+/i, "")
      .trim();
    const finalCleanName = cleanRawName
  ...
  // Carga los proyectos reales desde MongoDB
  ...
  }, [token, loadBackendProjects]);

  const handleAuthSuccess = (loggedUser, authToken) => {
    setUser(loggedUser);
    setToken(authToken);
    setView("dashboard");
    // Ocultar overlay tras auth exitosa
    setLoadingOverlay({ visible: false, message: "" });
  };

  const handleAuthLoading = (isLoading, msg) => {
    setLoadingOverlay({ visible: isLoading, message: msg || "" });
  };

  const handleLogout = async () => {
    setLoadingOverlay({ visible: true, message: "Cerrando sesión..." });
  ...
  // Si no está autenticado, renderizar la pantalla de autenticación
  if (!token || !user) {
    return (
      <>
        <LoadingOverlay visible={loadingOverlay.visible} message={loadingOverlay.message} />
        <AuthView onAuthSuccess={handleAuthSuccess} onLoadingChange={handleAuthLoading} />
      </>
    );
  }

  const createNewDraftProject = (id) => ({
  ...
  const handleSelectProject = async (projectId) => {
    setLoadingOverlay({ visible: true, message: "Preparando espacio de trabajo..." });
    setActiveProjectId(projectId);

    const targetBackendId = (projectId && projectId.length === 24)
      ? projectId
      : projects.find((p) => p.id === projectId)?.backendId;

    if (targetBackendId && targetBackendId.length === 24) {
      try {
        const fullProj = await fetchProjectById(targetBackendId);
        if (fullProj) {
          const mapped = mapBackendProject(fullProj);
          handleUpdateProject(mapped);
        }
      } catch (err) {
        console.warn("[App] Error cargando proyecto completo por ID:", err);
      }
    }

    // Pequeña pausa para que el overlay se vea fluido
    await new Promise((r) => setTimeout(r, 700));
    setView("workspace");
    setLoadingOverlay({ visible: false, message: "" });
  };

  const handleCreateProject = async (draftProject) => {
    setLoadingOverlay({ visible: true, message: "Creando proyecto..." });
    const proj = draftProject || createNewDraftProject();
  ...
  const handleDeleteProject = async (projectId) => {
    const proj = projects.find((p) => p.id === projectId || p.backendId === projectId);
    ...
    const filtered = projects.filter((p) => p.id !== projectId && p.backendId !== projectId);
    setProjects(filtered);
    if (activeProjectId === projectId || activeProjectId === targetBackendId) {
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-[#f8fafd] text-slate-900 overflow-hidden font-sans select-text">
      ...
      <div className="flex-1 flex overflow-hidden">
        {view === "dashboard" ? (
          <ProjectDashboard
            projects={projects.filter((p) => p.backendId || (p.id && !p.id.startsWith("draft-")))}
          ...
          <ProjectWorkspace
            ...
            onBackToDashboard={handleGoToProjects}
          />
        )}
      </div>
    </div>
